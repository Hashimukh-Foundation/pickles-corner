...
function ProductCard({ product }) {
	...
	const cheapestSize = sizes
		...
		.sort(
			(a, b) =>
				getFinalPrice(a, product.price) - getFinalPrice(b, product.price),
		)[0];

	const minFinalPrice = sizes.length
		? sizes.reduce((min, s) => {
				const p = getFinalPrice(s, product.price);
				return p < min ? p : min;
			}, Infinity)
		: product.price;
	const minOriginalPrice = sizes.length
		? sizes.reduce((min, s) => {
				const p = getOriginalPrice(s, product.price);
				return p < min ? p : min;
			}, Infinity)
		: product.price;
	...
	return (
		<div
			className={`bg-[#191715] border border-[#333333] hover:border-[#1F8B4D] transition-colors duration-300 flex flex-col group relative ${!inStock ? "opacity-75" : ""}`}
		>
			<Link
				to={`/products/${product.id}`}
				className="block relative aspect-square overflow-hidden bg-[#201e1c]"
			>
				{product.image_url ? (
					<img
				...
				<div className="absolute top-2 left-2 flex flex-col gap-2 z-10">
					...
					{inStock && bestDiscount && (
						<span className="bg-[#C62020] text-white text-xs font-bold px-2 py-1 uppercase tracking-widest shadow-md font-bangla-sans">
							{t("SALE", "অফার")}{" "}
							{isBn
								? toBn(bestDiscount.discount_percent)
								: bestDiscount.discount_percent}
							% {t("OFF", "ছাড়")}
						</span>
					)}
				</div>
			...
			<div className="p-4 text-center flex flex-col flex-grow">
				{(product.category || product.category_bn) && (
					<span
						className={`text-[#1F8B4D] text-[10px] font-bold uppercase tracking-widest mb-1 ${isBn ? "font-bangla-sans" : ""}`}
					>
				...
				<h3
					className={`text-xl font-bold text-white mb-2 leading-tight font-bangla-sans`}
				>
					<Link
						to={`/products/${product.id}`}
						className="hover:text-[#1F8B4D] transition-colors"
					>
				...
				<div className="mb-4 flex flex-col items-center justify-center gap-1 font-bangla-sans">
						...
						{bestDiscount ? (
							<>
								<span className="text-gray-500 line-through text-sm">
									Tk{" "}
									{isBn ? toBn(minOriginalPrice) : minOriginalPrice.toFixed(0)}
								</span>
								<span className="text-[#C62020] text-lg font-bold">
									Tk {isBn ? toBn(minFinalPrice) : minFinalPrice.toFixed(0)}
								</span>
							</>
						) : (
							<>
					...
					<span className="text-[10px] text-gray-500 uppercase tracking-widest">
						{isBn
							? `${toBn(sizes.length)} ${BN.sizesAvailable}`
							: `${sizes.length} sizes`}
					</span>
				</div>
				...
				<div className="mt-auto">
					{inStock ? (
						...
						<button
							disabled
							className="w-full bg-[#333333] text-gray-500 font-bold py-3 px-4 uppercase text-sm tracking-wide cursor-not-allowed border border-[#444] font-bangla-sans"
						>
...
export default function Home() {
	...
	useEffect(() => {
		supabase
			.from("products")
			.select(
				"*, product_sizes(id, size_grams, price_override, stock_quantity, discount_percent, discount_expires_at)",
			)
			.eq("is_active", true)
			.limit(8)
	...
	const saleProducts = products.filter((p) =>
		getBestDiscount(p.product_sizes ?? []),
	);
	...
	return (
		<main className="bg-black min-h-screen">
			...
			<div className="relative bg-black mb-12 border-b border-[#2A2A2A] h-[400px] sm:h-[500px] overflow-hidden group">
				<img
					src="https://i.postimg.cc/g2VNQmkJ/1.jpg"
					alt="Hero Background"
					onError={(e) => (e.currentTarget.style.display = "none")}
				...
				<div className="relative z-10 px-6 h-full flex flex-col items-center justify-center text-center">
					<span
						className={`inline-block text-[#1F8B4D] text-xs font-bold uppercase tracking-widest mb-4 ${isBn ? "font-bangla-sans" : ""}`}
					>
						{t("Premium Quality", BN.premiumQuality)}
					</span>
					<h1
						className={`text-4xl md:text-6xl text-white mb-4 font-bold uppercase tracking-tighter drop-shadow-2xl font-bangla-sans`}
					>
						{t("Authentic", "খাঁটি")}&nbsp;
						<span className="text-[#1F8B4D]">{t("Taste", "স্বাদ")}</span>
					</h1>
					<p
						className={`text-sm md:text-base text-gray-200 mb-8 max-w-xl mx-auto font-light tracking-wide font-bangla-serif`}
					>
						{t(
							"Handcrafted pickles made from secret family recipes. Sun-dried, spiced to perfection.",
							"পারিবারিক গোপন রেসিপিতে তৈরি খাঁটি আচার। রোদে শুকানো এবং সম্পূর্ণ মশলাদার।",
						)}
					</p>
					<div className="flex flex-wrap justify-center gap-4">
						<Link
							to="/products"
							className={`inline-block bg-[#1F8B4D] hover:bg-[#166E3B] text-white font-bold py-3 px-10 transition-all border border-transparent hover:border-green-400 uppercase tracking-widest text-xs ${isBn ? "font-bangla-sans" : ""}`}
						>
							{t("Shop Now", BN.shopNow)}
						</Link>
						<Link
							to="/blogs"
							className={`inline-block bg-transparent text-white font-bold py-3 px-10 transition-all border border-[#333] hover:border-white uppercase tracking-widest text-xs ${isBn ? "font-bangla-sans" : ""}`}
						>
			...
			{saleProducts.length > 0 && (
				<div className="bg-[#1c1a18] border-b border-[#333] py-3 mb-12">
					<div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center gap-4 overflow-x-auto">
						<span
							className={`text-[10px] font-black uppercase tracking-widest whitespace-nowrap bg-[#C62020] text-white px-3 py-1 shadow-md flex-shrink-0 font-bangla-sans`}
						>
							🔥 {t("ON SALE", BN.onSale)}
						</span>
						<div className="flex items-center gap-6 text-xs font-medium uppercase tracking-wider whitespace-nowrap font-bangla-sans">
							{saleProducts.map((p) => {
								...
								return (
									...
									>
										<span className="text-[#1F8B4D]">
											{isBn
												? toBn(best.discount_percent)
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-20 font-bangla-sans">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
						{categories.map((cat) => (
							<div
								key={cat.id}
								className="relative h-64 group overflow-hidden border border-[#333] bg-[#1e1c1a]"
							>
								...
								{cat.image_url && (
									<img
										...
										onError={(e) => (e.currentTarget.style.display = "none")}
										alt={t(cat.name, cat.name_bn)}
									/>
								)}
								<div className="absolute inset-0 z-20 flex flex-col items-center justify-center">
									<h3 className="text-2xl font-bold text-white uppercase tracking-widest mb-2">
										{t(cat.name, cat.name_bn)}
									</h3>
									<Link
										to="/products"
										className="text-[#1F8B4D] text-xs font-bold uppercase tracking-widest border-b border-[#1F8B4D] pb-1 hover:text-white hover:border-white transition-colors"
									>
										{t("View Collection", "কালেকশন দেখুন")}
									</Link>
								</div>
							</div>
						))}
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-24">
				<div className="flex items-center justify-between mb-8">
					<h2
						className={`text-xl font-bold text-white uppercase tracking-wide border-l-4 border-[#1F8B4D] pl-4 font-bangla-sans`}
					>
						{t("Shop Favorites", BN.ourProducts)}
					</h2>
					<Link
						to="/products"
						className={`text-xs text-gray-500 uppercase tracking-widest hover:text-white transition-colors font-bangla-sans`}
					>
				...
				{products.length > 0 ? (
					...
					<p className="text-gray-500 text-sm uppercase tracking-widest font-bangla-sans">
						{t("No products available yet.", "এখনো কোনো পণ্য নেই।")}
					</p>
				)}
			</div>
				...
				<div className="bg-[#1c1a18] border-y border-[#333] py-16 mb-24">
					<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
						<div className="text-center mb-10">
							<h2
								className={`text-2xl font-bold text-white uppercase tracking-wide mb-2 font-bangla-sans`}
							>
								{t("What People Say", BN.customerReviews)}
							</h2>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
							{reviews.map((r) => (
								<div
									key={r.id}
									className="bg-[#191715] p-6 border border-[#333] relative"
								>
									<div className="text-[#1F8B4D] text-3xl absolute top-4 right-6 font-serif opacity-20">
										"
									</div>
									<div className="flex text-[#1F8B4D] mb-3 text-sm">
										{[1, 2, 3, 4, 5].map((i) => (
											<span
												key={i}
												className={
													i <= r.rating ? "text-[#1F8B4D]" : "text-gray-700"
												}
											>
												★
											</span>
									...
									<div className="flex items-center gap-3">
										...
										<div>
											<h4 className="font-bangla-sans text-white text-xs font-bold uppercase tracking-wide">
												{r.customer_name}
											</h4>
											{r.products?.name && (
												<p className="font-bangla-sans text-gray-500 text-[10px] uppercase tracking-widest">
													{t(r.products.name, r.products.name_bn)}
												</p>
											)}
										</div>
									</div>
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
						{blogs.map((b) => (
							<div
								key={b.id}
								className="bg-[#191715] border border-[#333333] hover:border-[#1F8B4D] transition-colors duration-300 flex flex-col group relative p-8"
							>
